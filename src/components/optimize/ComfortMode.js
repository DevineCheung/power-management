...
const ComfortMode = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await fetch('/optimization/comfort.csv');
            ...
            Papa.parse(text, {
                ...
                complete: (result) => {
                    // 跳过第一行数据（标题行）
                    const validData = result.data.slice(1).filter(row => row.length > 1);

                    // 生成电价曲线数据
                    const priceData = Array(144).fill(0);
                    pricePeriods.forEach(({ start, end, price }) => {
                        const startIdx = start * 6;
                        const endIdx = end * 6;
                        priceData.fill(price, startIdx, endIdx);
                    });
    ...
    return (
        <div style={{ width: '98%', margin: '2px auto' }}>
            <h3 style={{ marginBottom: '0px', marginTop: '0px' }}>舒适模式</h3>
